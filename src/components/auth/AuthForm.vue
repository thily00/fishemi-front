...
<template>
    <div class="rounded-xl p-10 flex flex-col justify-between bg-blue w-full">
        <p class="fishemi-text-color font-bold xl:text-5xl lg:text-4xl md:text-3xl xs:text-2xl text-2xl mb-8">
        ...
        <div class="flex flex-col gap-2 text-white">
            ...
            <div>
                <FishemiButton label="Connexion" fullWidth="true" :action="() => redirectTo('dashboard')" />
                <p class="mt-3 cursor-pointer">
                    {{ props.isLogin ? "Vous n'avez pas de compte ?" : "Vous avez déja un compte ?" }}
                    <span class="fishemi-text-color font-bold" @click="redirectTo()">{{ props.isLogin ?  "S'inscrire" : "Se connecter" }}</span>
                </p>
            </div>
        </div>
    </div>
