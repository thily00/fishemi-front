...
<template>
  <div class="w-full bg-background rounded-md p-5 transition-all duration-300">
    ...
    @before-leave="beforeLeave">
      <div v-if="!isOpen" class="w-full flex items-center justify-between">     
        <div class="flex items-center gap-6 rounded-md">
          <Checkbox :modelValue="listStore.selectedLists.includes(props.id)" @update:modelValue="handleCheckboxChange" :binary="true" class="bg-blue rounded-sm" />
          <i class="pi pi-users fishemi-text-color" style="font-size: 1.75rem"></i>
          <div>
            <h1 class="fishemi-text-color text-xl mb-1">{{ props.name }}</h1>
            <h6 class="text-gray-400" v-if="props.employee_count && props.employee_count <= 1">{{ props.employee_count }} membre</h6>
            <h6 class="text-gray-400" v-else>{{ props.employee_count }} membres</h6>
          </div>
        </div>
    ...
    @before-leave="beforeLeave" 
    @leave="leave">
      <div v-if="isOpen" class="w-full flex flex-col" style="overflow: hidden; transition: max-height 0.3s ease;">
        ...
        <div class="form flex flex-col flex-start my-4" v-if="listStore.selectedList">
          <div class="flex flex-col my-6 gap-4">
            <div class="w-full flex flex-col gap-4 text-white">
              <label for="name">Nom de la liste</label>
              <InputText
                id="name"
                class="mb-2 bg-background border-slate-700" v-model="listStore.selectedList.name" disabled />
            </div>
            <div class="w-full flex flex-col gap-4 text-white">
                ...
                <div class="w-96">
                    <IconField>
                        ...
                        <InputText placeholder="Rechercher un employé..." class="border-0 bg-blue p-4" />
                    </IconField>
                </div>
                <ul class="flex gap-4">
                    <li v-for="employee in props.employeeList" :key="employee.id" 
                    class="flex flex-col items-start gap-1 bg-blue rounded-md p-4">
                        <Checkbox :modelValue="listEmployees.includes(employee.id)" @update:modelValue="handleEmployeeCheckboxChange(employee.id)" :binary="true" class="bg-background rounded-md" />
                        <h1 class="fishemi-text-color text-lg mb-0">{{ employee.full_name }}</h1>
